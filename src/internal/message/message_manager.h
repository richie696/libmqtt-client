...
struct QueuedMessage {
    ...
    MqttProperties properties;  ///< MQTT 5.0属性
...
public:
    ...
    /**
     * @brief 发布带MQTT 5.0属性的消息（沿用同一队列/持久化/重试路径）
     */
    [[nodiscard]] Result<bool> publishWithProperties(const std::string& topic,
                                                      const std::string& payload,
     ...
     * @param qos QoS等级
     * @param retained 是否保留
     * @param timeoutMs 保留参数；实际网络超时由连接配置控制
     * @return Result<bool> 发布结果
     */
...
private:
    ...
    [[nodiscard]] Result<bool> publishMessage(QueuedMessage msg);
