/**
 * @file test_mqtt5_tcp_tls.cpp
 * @brief MQTT 5.0 TCP和TLS协议收发消息测试
 * 
 * 测试MQTT 5.0协议在TCP和TLS连接下的消息发送和接收功能
...
MqttConfig createTcpConfig() {
    ...
    config.basic.clientId = "mqtt5_tcp_test_" + std::to_string(std::time(nullptr));
    config.basic.version = "5.0";  // 使用MQTT 5.0
...
bool testTcpQoS(QoS qos) {
    ...
    config.basic.clientId = "mqtt5_tcp_qos" + std::to_string(static_cast<int>(qos)) + "_" + 
    ...
    // 发布消息
    std::string testMessage = "MQTT 5.0 TCP " + qosToString(qos) + " Test - " + 
                              std::to_string(std::time(nullptr));
    ...
    if (waitForMessages(&msgState, timeout)) {
        // 等待一小段时间，确保消息被完全添加到列表中
...
bool testTlsQoS(QoS qos) {
    ...
    config.basic.clientId = "mqtt5_tls_qos" + std::to_string(static_cast<int>(qos)) + "_" + 
                            std::to_string(std::time(nullptr));
    EmbeddedMqttClient client(config);
    ...
    // 发布消息
    std::string testMessage = "MQTT 5.0 TLS " + qosToString(qos) + " Test - " + 
                              std::to_string(std::time(nullptr));
...
TcpTestResults testTcpConnection() {
    std::cout << "\n========================================" << std::endl;
    std::cout << "测试 1: MQTT 5.0 TCP 连接" << std::endl;
    std::cout << "========================================" << std::endl;
...
TlsTestResults testTlsConnection() {
    std::cout << "\n========================================" << std::endl;
    std::cout << "测试 2: MQTT 5.0 TLS 连接" << std::endl;
    std::cout << "========================================" << std::endl;
...
int main(int /*argc*/, char* /*argv*/[]) {
    std::cout << "========================================" << std::endl;
    std::cout << "MQTT 5.0 TCP和TLS协议收发消息测试" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "协议版本: MQTT 5.0" << std::endl;
